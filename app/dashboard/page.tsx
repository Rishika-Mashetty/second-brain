...
export default function Dashboard() {
  ...
  const [filter, setFilter] = useState<
    'all' | 'youtube' | 'x' | 'instagram' | 'linkedin' | 'github' | 'notes'
  >('all');
  const [sidebarOpen, setSidebarOpen] = useState(true);
  ...
  const addPost = async (e: any) => {
    e.preventDefault();
    if (!url && !comment.trim()) return alert('Enter a note or a link!');
    const res = await fetch('/api/posts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url, comment }),
    ...
    const j = await res.json();
    if (j.post) setPosts([j.post, ...posts]);
    setUrl('');
    setComment('');
  };
  ...
  const editPost = async (id: string) => {
    ...
    const newComment = prompt('Edit comment:', post?.comment || '');
    const res = await fetch('/api/posts', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, url: newUrl, comment: newComment }),
  ...
  const filterPosts = (p: any) => {
    const u = (p.url || '').toLowerCase();
    if (filter === 'all') return true;
    if (filter === 'notes') return !p.url; // 📝 show only notes
    if (!u) return false;
    if (filter === 'youtube') return u.includes('youtube.com') || u.includes('youtu.be');
    if (filter === 'x') return u.includes('x.com') || u.includes('twitter.com');
    if (filter === 'instagram') return u.includes('instagram.com');
      ...
      <main className="flex-1 p-4 md:p-6 overflow-y-auto transition-all duration-300">
        ...
        <form onSubmit={addPost} className="flex flex-col md:flex-row gap-3 mb-8 items-center">
          <input
            value={url}
            onChange={(e) => setUrl(e.target.value)}
            placeholder="Paste a link (YouTube, X, GitHub...) — or leave blank for a note"
            className="flex-1 p-3 rounded bg-white/10"
          />
          <input
            value={comment}
        ...
        <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4">
          {posts.filter(filterPosts).map((p) => (
            <div
              key={p._id}
              className={cn(
                'break-inside-avoid border rounded-lg p-3 mb-4 transition-all',
                p.url
                  ? 'border-white/10 bg-white/5 hover:bg-white/10'
                  : 'border-yellow-300/30 bg-yellow-200/10 hover:bg-yellow-200/20'
              )}
            >
              {p.url ? (
                <>
                ...
                <div className="p-2">
                  <p className="text-base font-medium text-yellow-200 whitespace-pre-wrap leading-relaxed">
                    📝 {p.comment}
                  </p>
                </div>
              )}
